...
// FIX: AuthService.login now falls back to Firestore "users" collection for
//      role & linkedId if Firebase Custom Claims are not set — works without
//      Cloud Functions backend. Just run seedFirestore.js once.
...
export const AuthService = {
  // Login — returns Firebase user + role + profile
  // Priority: 1) Firebase Custom Claims  2) Firestore "users" collection
  login: async (email, password) => {
    ...
    // ── Fallback: read role from Firestore users collection ──────────────────
    // This is the key fix — no backend/Cloud Functions needed.
    // The seedFirestore.js script writes role+linkedId to users/{uid} in Firestore.
    ...
    const profile    = await one(docRef(profileCol, userLinked));
    return { user: cred.user, role: userRole, linkedId: userLinked, profile };
  },
  ...
  // Get fresh token with custom claims (returns null if not set)
  getRole: async () => {
...
export const StudentService = {
  getAll: async (filters = {}) => {
    let constraints = [];
    // Only IT and CS (CCS department only)
    if (filters.deptId) {
      constraints.push(where("deptId","==",filters.deptId));
    } else {
      // Default: only IT and CS
      constraints.push(where("deptId","in",["CS","IT"]));
    }
    ...
    const q = query(col("students"), ...constraints, orderBy("lastName"));
...
export const AdminService = {
  getDashboardStats: async () => {
    const [students, faculty, events, research] = await Promise.all([
      snap(query(col("students"), where("status","==","active"), where("deptId","in",["CS","IT"]))),
      snap(query(col("faculty"), where("deptId","in",["CS","IT"]))),
  ...
  onStatsChange: (callback) =>
    onSnapshot(query(col("students"), where("status","==","active"), where("deptId","in",["CS","IT"])), () => {
